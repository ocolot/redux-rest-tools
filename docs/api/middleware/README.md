...
Returns **[object](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object)** The normalized data as an immutable object with the following keys: `result`, the list of object keys (see `idPath`) and `entities`, an object with each key containing the related entity.
